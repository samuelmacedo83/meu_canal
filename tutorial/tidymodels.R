...
#recipe
reg_recipe <- recipe(Petal.Length ~ ., data = iris_trainning) %>% 
  step_impute_knn(all_predictors()) %>% 
  step_dummy(all_nominal_predictors()) %>% 
  step_normalize(all_numeric_predictors())

rf_recipe <- recipe(Petal.Length ~ ., data = iris_trainning) %>% 
  step_impute_knn(all_predictors()) 
...
val_set <- vfold_cv(iris_trainning, v = 4, strata = Species)
...
rf_trained %>% show_best(n = 10) %>% as.data.frame()
